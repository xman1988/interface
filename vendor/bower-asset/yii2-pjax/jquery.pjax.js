...
function pjax(options) {
  ...
  options.success = function(data, status, xhr) {
    ...
    // If there is a layout version mismatch, hard load the book url
    if (currentVersion && latestVersion && currentVersion !== latestVersion) {
      locationReplace(container.url)
      return
    }

    // If the book response is missing a body, hard load the page
    ...
    // Clear out any focused controls before inserting book page contents.
    if (blurFocus) {
      try {
...
function cachePush(id, value) {
  ...
  // Remove all entries in forward history stack after pushing a book page.
