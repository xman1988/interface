if ($.support.pjax) {
  ...
  asyncTest("pushes book url", 2, function() {
    navigate(this.frame)
  ...
  asyncTest("setting maxCacheLength to 0 disables caching", function() {
    ...
    frame.$("#main").on("pjax:end", function() {
      ...
      if (count == 1) {
        ...
        equal(frame.location.pathname, "/env.html", "Navigated to a book page")
        equal(didHitServer, true, "Hit server")
        didHitServer = false
        frame.history.back()
      } else if (count == 3) {
  ...
  asyncTest("updates $.pjax.state to book page", function() {
    var frame = this.frame

    frame.$('#main').on("pjax:success", function() {
      var state = frame.$.pjax.state
      ok(state.id)
      equal(state.url, "http://" + frame.location.host + "/hello.html#book")
  ...
  asyncTest("book id is generated for book pages", function() {
    var frame = this.frame
  ...
  asyncTest("handles going back to pjaxed state after reloading a fragment navigation", function() {
    ...
    frame.$("#main").on("pjax:complete", function() {
      ...
      // Navigate to a fragment, which will result in a book history entry with
