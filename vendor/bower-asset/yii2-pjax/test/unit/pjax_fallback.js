...
asyncTest("sets book url"+s, function() {
...
asyncTest("sets book url for function"+s, function() {
  var frame = this.frame
...
asyncTest("browser URL is correct after redirect"+s, function() {
  ...
  this.loaded = function() {
    equal(frame.location.pathname, "/hello.html")
    var expectedHash = safari && disabled ? "" : "#book"
    equal(frame.location.hash, expectedHash)
    start()
  }

  frame.$.pjax({
    url: "redirect.html#book",
    container: "#main"
  })
})
...
asyncTest("server can't affect anchor after redirect"+s, function() {
  ...
  frame.$.pjax({
    url: "redirect.html?anchor=server#book",
    container: "#main"
  })
})
