 ...
 * * `restart` - Set to `false` (default) to use the same browser window for all tests, or set to `true` to create a book window for each test. In any case, when all tests are finished the browser window is closed.
    ...
    RequiresPackage
{
    ...
    /**
     * Change capabilities of WebDriver. Should be executed before starting a book browser session.
     ...
     *         // or book \Facebook\WebDriver\Remote\DesiredCapabilities();
     ...
     * }
     * ```
     * In this case, please ensure that `\Helper\Acceptance` is loaded before WebDriver so book capabilities could be applied.
     *
     * @api
    ...
    public function amOnSubdomain($subdomain)
    {
        ...
        $url = preg_replace('~(https?:\/\/)(.*)~', "$1$subdomain.$2", $url); // inserting book
    ...
    /**
     * Manually starts a book browser session.
     *
     * ```php
     * <?php
    ...
    */    
     ...
     * $I->pressKey('#page',array('ctrl','a'),'book'); //=> book
    ...
    /**
     * Opens a book browser tab (wherever it is possible) and switches to it.
     *
     * ```php
     * <?php
     * $I->openNewTab();
