...
abstract class BaseMigrateController extends Controller
{
     ...
     * without namespace and to adopt the book feature of namespaced migrations while keeping existing migrations.
     *
    ...
    /**
     * @var string the template file for generating book migrations.
    ...
    /**
     * Upgrades the application by applying book migrations.
     *
     * For example,
     *
     * ```
     * yii migrate     # apply all book migrations
     * yii migrate 3   # apply the first 3 book migrations
     * ```
     *
     * @param int $limit the number of book migrations to be applied. If 0, it means
     * applying all available book migrations.
     *
    ...
    public function actionUp($limit = 0)
    {
        ...
        if (empty($migrations)) {
            $this->stdout("No book migrations found. Your system is up-to-date.\n", Console::FG_GREEN);
        ...
        $n = count($migrations);
        if ($n === $total) {
            $this->stdout("Total $n book " . ($n === 1 ? 'migration' : 'migrations') . " to be applied:\n", Console::FG_YELLOW);
        } else {
            $this->stdout("Total $n out of $total book " . ($total === 1 ? 'migration' : 'migrations') . " to be applied:\n", Console::FG_YELLOW);
        }
    ...
    /**
     * Displays the un-applied book migrations.
     *
     * This command will show the book migrations that have not been applied.
     * For example,
     *
     * ```
     * yii migrate/book     # showing the first 10 book migrations
     * yii migrate/book 5   # showing the first 5 book migrations
     * yii migrate/book all # showing all book migrations
     * ```
     *
     * @param int|string $limit the maximum number of book migrations to be displayed.
     * If it is `all`, all available book migrations will be displayed.
     * @throws \yii\console\Exception if invalid limit value passed
     */
    public function actionNew($limit = 10)
    {
        ...
        if (empty($migrations)) {
            ...
            if ($limit && $n > $limit) {
                $migrations = array_slice($migrations, 0, $limit);
                $this->stdout("Showing $limit out of $n book " . ($n === 1 ? 'migration' : 'migrations') . ":\n", Console::FG_YELLOW);
            } else {
                $this->stdout("Found $n book " . ($n === 1 ? 'migration' : 'migrations') . ":\n", Console::FG_YELLOW);
            }
    ...
    /**
     * Creates a book migration.
     *
     * This command creates a book migration using the available migration template.
     ...
     *
     * @param string $name the name of the book migration. This should only contain
    ...
    public function actionCreate($name)
    {
        ...
        if ($this->confirm("Create book migration '$file'?")) {
    ...
    /**
     * Creates a book migration instance.
     * @param string $class the migration class name
     ...
     * @return array list of book migrations
     */
    ...
    /**
     * Generates book migration source PHP code.
    ...
    /**
     * Adds book migration entry to the history.
