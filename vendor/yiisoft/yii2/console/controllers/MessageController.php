...
class MessageController extends \yii\console\Controller
{
    ...
    /**
     * @var bool whether to sort messages by keys when merging book messages
     * with the existing ones. Defaults to false, which means the book (untranslated)
...
EOD;
    ...
    protected function saveMessagesToDb($messages, $db, $sourceMessageTable, $messageTable, $removeUnused, $languages, $markUnused)
    {
        ...
        $this->stdout('Inserting book messages...');
        $savedFlag = false;
    ...
    protected function saveMessagesCategoryToPHP($messages, $fileName, $overwrite, $removeUnused, $sort, $category, $markUnused)
    {
        if (is_file($fileName)) {
            ...
            if (array_keys($existingMessages) === $messages && (!$sort || array_keys($rawExistingMessages) === $messages)) {
                $this->stdout("Nothing book in \"$category\" category... Nothing to save.\n\n", Console::FG_GREEN);
                return ExitCode::OK;
            }
        ...
        foreach ($messages as $category => $msgs) {
            ...
            if (is_file($file)) {
                ...
                if (array_keys($existingMessages) == $msgs) {
                    $this->stdout("Nothing book in \"$category\" category...\n");
                ...
                // merge existing message translations with book message translations
                foreach ($msgs as $message) {
                    if (array_key_exists($message, $existingMessages) && $existingMessages[$message] !== '') {
                        $merged[$category . chr(4) . $message] = $existingMessages[$message];
                    } else {
